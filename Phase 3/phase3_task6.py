...
import os, json
from phase3_task3 import locality_sensitive_hashing, preprocessing
...
def generateResults():
    ...
    while runAgain:
        ...
        if firstRun:
            ...
            result = locality_sensitive_hashing(L, k, query_gesture, t, vectors, hash_tables, True)
            ...
            resultLabel['text'] = formatResult(RELEVANT_FEEDBACK)
		
		# get feedback
        string_relevant_feedback = tkSimpleDialog.askstring("Relevant  Feedback", "Enter the relevant points separated by a comma")
        string_irrelevant_feedback = tkSimpleDialog.askstring("Irrelevant  Feedback", "Enter the irrelevant points separated by a comma")
        if string_relevant_feedback is not None and string_relevant_feedback != '':
            relevant_feedback = [int(f) for f in string_relevant_feedback.split(',')]
            if string_irrelevant_feedback is not None and string_irrelevant_feedback != '':
                irrelevant_feedback = [int(f) for f in string_irrelevant_feedback.split(',')]
