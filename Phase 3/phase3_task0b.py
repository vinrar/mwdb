...
import ast
import sys
# from create_vector_matrix import generate_vector_matrix

tf_idf_dict = {}

# returns list of .wrd files
def get_files_list(dir):
    list_of_files = os.listdir(dir)
    return [ file for file in list_of_files if file.endswith('.wrd')]

# returns count of words across all dimensions
...
# returns the occurrence of a word across all sensors for a given dimension
...
# calculates and stores the tfidf vector for each file
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 1:
        print('Run python phase3_task0b.py <Directory> ')
        sys.exit(0)
    ...
    # reads data from .wrd files and creates tf and tfidf vectors
